...
import { Package2, ShoppingCart, Receipt, DollarSign, Loader2 } from "lucide-react";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if(user?.factory_selected?.id){
        const response = await fetch(`/api/stock-product?factory_id=${user?.factory_selected?.id}`);
            ...
            <Table className="border">
              <TableHeader className="border-b font-bold uppercase text-bal">
                <TableRow className="divide-x">
                  <TableHead rowSpan={2} className="font-bold text-black text-center">No</TableHead>
                  <TableHead rowSpan={2} className="font-bold text-black">Nama Produk</TableHead>
                  <TableHead rowSpan={2} className="font-bold text-black">Tipe</TableHead>
                  <TableHead rowSpan={2} className="font-bold text-black">Harga</TableHead>
                  <TableHead rowSpan={2} className="font-bold text-black">Stok Terjual</TableHead>
                  <TableHead colSpan={4} className="text-center font-bold text-black">Stok Tersedia</TableHead>
                </TableRow>
                <TableRow>
                  <TableHead className="text-black font-bold text-end border-r border-l">Pack</TableHead>
                  <TableHead className="text-black font-bold text-end border-r">Slop/Press</TableHead>
                  <TableHead className="text-black font-bold text-end border-r">Bal</TableHead>
                  <TableHead className="text-black font-bold text-end border-r">Karton</TableHead>
                </TableRow>
              </TableHeader>
