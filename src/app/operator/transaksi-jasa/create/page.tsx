...
import { CalendarIcon, Check, Search, SearchCheckIcon, Trash } from "lucide-react";
import {
  Select,
  SelectTrigger,
  SelectValue,
...
export default function TransaksiJasa() {
  ...
  const addToCart = () => {
    ...
    const newItem: any = {
      ...
      subtotal_discount: quantity * price - disconPrice,
    };

    // jika sudah ada service di cart, maka update quantity dan subtotal
    const existingItem = cart.find((item: any) => item.service === selectedService);
    if (existingItem) {
  ...
  const handleSave = async () => {
    ...
    const response = await fetch('/api/transaction-service', {
      method: 'POST',
      headers: {
    ...
    if(!response.ok) {
      ...
      router.push("/operator/transaksi-jasa");
      // clear cart
      setCart([]);
      setBuyer({
        name: "",
                      ...
                      <TableBody>
                        {getCurrentServicePageData().map((service: any) => (
                          <TableRow key={service.id}>
                            ...
                            <TableCell>Rp. {formatIDR(service.price)}</TableCell>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {cart.map((item: any) => (
                    <tr key={item.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                        {formatIDR(item.price * item.quantity - item.discount)}
                      </td>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <div>
                ...
                <Input
                  ...
                  value={formatIDR(paymentDetails.downPayment)}
                  onChange={(e) =>
                    setPaymentDetails((prev) => ({
                      ...prev,
              ...
              <div>
                ...
                <Input
                  ...
                  value={formatIDR(paymentDetails.remainingPayment)}
                  readOnly
                />
              </div>
              <div>
