  ...
  return (
    <div className="p-8 font-sans bg-white text-gray-800">
        ...
        <tbody>
          {data.DetailOrderMaterialUnit.map((detail: any, index: number) => (
            <tr key={detail.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
              ...
              <td className="border border-gray-300 px-2 py-1 text-right">{detail.price}</td>
              <td className="border border-gray-300 px-2 py-1 text-right">{detail.amount * detail.price}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="text-right mt-6">
        <p className="text-lg font-bold">Total Harga Item: Rp {data.amount * data.price}</p>
      </div>
    </div>
  );
}
