    ...
    cell: ({ row }) => (
      <div className="text-start">
        {row.original.DetailOrderBahanBakuDistributor?.map((item: any) => (
          <div key={item.id}>
            {item.material_distributor?.name} /{" "}
            {item.material_distributor?.unit?.name}
          </div>
        ))}
      </div>
    ),
                      ...
                      {row.original.DetailOrderBahanBakuDistributor?.map(
                        (detail: any, index: number) => (
                          <tr key={index} className="border-t">
                            ...
                            <td className="px-4 py-2 text-right">
                              Rp{" "}
                              {new Intl.NumberFormat('id-ID', { style: 'decimal', currency: 'IDR' }).format(detail.sub_total)}
                            </td>
                          </tr>
                        )
                      )}
                    ...
                    <tfoot className="bg-gray-50">
                      <tr>
                        ...
                        <td className="px-4 py-2 font-bold text-right">
                          Rp{" "}
                          {new Intl.NumberFormat('id-ID', { style: 'decimal', currency: 'IDR' }).format(row.original.DetailOrderBahanBakuDistributor?.reduce(
