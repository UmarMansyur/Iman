...
export default function UploadBukti({ data, fetchProducts }: { data: any, fetchProducts: any }) {
  ...
  const handleSave = async (id: number) => {
    try {
      ...
      const response = await fetch(`/api/distributor/pelunasan`, {
        method: "PATCH",
        body: formData,
      });
              ...
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  ...
                  <span className="text-sm text-blue-500 font-bold">
                    {" "}
                    Jumlah yang Harus Dibayar ({formatCurrency(data.remaining_balance)})
                  </span>
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 w-1/2 mx-auto">
                <div className="border-2 border-dashed rounded-lg p-4 bg-gray-50">
                  {previewImage || data.proof_of_payment_2 ? (
                    <div className="relative w-full aspect-square">
                      <Image
                        src={previewImage || data.proof_of_payment_2}
